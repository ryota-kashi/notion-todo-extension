...
- ✅ **Notionページへのアクセス**: Notionアイコンをクリックして、タスクのNotionページを直接開く
- ✅ **Googleカレンダー連携**: カレンダーアイコンをクリックして、タスク情報が事前入力されたGoogleカレンダーの新規イベント画面を開く（設定不要）
...
- 🔍 **高度なフィルター機能**: タグ、ステータス、優先度などでタスクを柔軟に絞り込み（AND条件対応）
...
4. 必要に応じて、表示したいプロパティを選択
   - ※Notion側でプロパティを追加した場合は、編集画面の「🔄 情報更新」ボタンを押してください
5. **フィルター設定**:
   - データベース編集画面の下部で、表示するタスクの絞り込み条件（例：「タグ」が「重要」など）を設定できます。
   - **重要**: 担当者やタグの選択肢を取得するには、右上の **「🔄 更新」** ボタンを押してください。
   - 結合条件を「すべて一致 (AND)」か「いずれか一致 (OR)」から選択できます。
